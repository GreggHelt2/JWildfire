...
*/
public class RhodoneaSimpleFunc extends VariationFunc {
  ...
  public void init(FlameTransformationContext pContext, Layer pLayer, XForm pXForm, double pAmount) {
    ...
    if ((k % 1) == 0) { // k is integer 
      if ((k % 2) == 0) { // k is even integer, will have 2k petals
        ...
        if (radial_offset != 0 || inner_spread != 0 || outer_spread != 0 || thickness != 0) {
          cycles_to_close = 1;
  ...
  public void transform(FlameTransformationContext pContext, XForm pXForm, XYZPoint pAffineTP, XYZPoint pVarTP, double pAmount) {
    ...
    */
    ...
    double rin = spread_split * sqrt((xin * xin) + (yin * yin));
    ...
    if (abs(rin) > abs(r)) { // incoming point lies outside of current petal of rose curve
      switch (outer_mode) {
        case 0: // no spread, but factor in thickness
          if (thickness != 0) {
        ...
        case 2:
          ...
          //xout = x + ((rin - r) * outer_spread * outer_spread_ratio);
        ...
        case 6: // mask "outside" the curve
          ...
          pVarTP.doHide = true;
          break;
        case 7: // same as outer_mode 3, but without the sign modifications
          xout = (x + (outer_spread * outer_spread_ratio * xin));
          yout = (y + (outer_spread * yin));
          break;
          /*        case 6: 
    ...
    if (diff_mode) {
      pVarTP.x = pAffineTP.x + (adjustedAmount * (xout - pAffineTP.x));
      pVarTP.y = pAffineTP.y + (adjustedAmount * (yout - pAffineTP.y));
    }
    else {
      pVarTP.x += adjustedAmount * xout;
      pVarTP.y += adjustedAmount * yout;
    }
  ...
  public Object[] getParameterValues() {
    return new Object[] { knumer, kdenom,
                          ...
                          cycles_param, cycle_offset, metacycles, metacycle_expansion, (diff_mode ? 1 : 0)  };
  }
